...
This a compromise rather than the ideal case. We must trust that library internals correctly uphold all the invariants. Nlohmann's JSON library contains thousands of lines of code with access to the private fields of `json`, so this is definitely not a trivial concern!
...
We have reigned in the correctness of our data types using ADTs. Continuing the example from the previous section, we can, for example, define a function which returns a string if the JSON represents one:
